...
import { PhotoBackground } from "@/assets";
...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      setMensagemError("As senhas não coencidem");
    ...
    try {
      const response = await api.post("/balconista/register", {
  ...
  return (
    <section className="w-full h-screen flex items-end justify-end relative">
      <Image
        ...
        className="absolute inset-0 max-lg:hidden w-full h-full object-cover z-0"
      />
      <div className="bg-white w-[40%] px-4 max-lg:w-full flex flex-col items-center justify-center h-screen z-10 relative shadow-lg">
        ...
        >
          ...
          <div>
            <p className="text-[#fba900] font-[600] mt-3 text-[1.2rem]">
              Já tem uma conta?{" "}
              <Link href="/login" className="text-[#fba800]">
                clique aqui
              </Link>
            </p>
          </div>
